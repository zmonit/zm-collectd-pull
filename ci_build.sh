...
default|default-Werror|default-with-docs|valgrind)
    ...
    # Start of recipe for dependency: libcollectdclient
    if ! (command -v dpkg-query >/dev/null 2>&1 && dpkg-query --list libcollectdclient-dev >/dev/null 2>&1) || \
           (command -v brew >/dev/null 2>&1 && brew ls --versions libcollectdclient >/dev/null 2>&1) \
    ; then
        echo ""
        BASE_PWD=${PWD}
        echo "`date`: INFO: Building prerequisite 'libcollectdclient' from Git repository..." >&2

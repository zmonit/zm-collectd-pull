/*  =========================================================================
    zm_collectd_pull_selftest.c - run selftests
...
// Tests for draft public classes:
    { "zm_collectd_pull_actor", zm_collectd_pull_actor_test },
#endif // ZM_COLLECTD_PULL_BUILD_DRAFT_API
#ifdef ZM_COLLECTD_PULL_BUILD_DRAFT_API
    { "private_classes", zm_collectd_pull_private_selftest },
#endif // ZM_COLLECTD_PULL_BUILD_DRAFT_API
    {0, 0}          //  Sentinel
};
...
{
    test_item_t *item;
    printf ("Running zm-collectd-pull selftests...\n");
    for (item = all_tests; item->test; item++)
        item->test (verbose);
...
main (int argc, char **argv)
{
    ...
    for (argn = 1; argn < argc; argn++) {
        ...
        ||  streq (argv [argn], "-h")) {
            puts ("zm_collectd_pull_selftest.c [options] ...");
        ...
        ||  streq (argv [argn], "-l")) {
            ...
            puts ("    zm_collectd_pull_actor\t\t- draft");
    ...
    if (test) {
        printf ("Running zm-collectd-pull test '%s'...\n", test->testname);
        test->test (verbose);
    }
    else
        test_runall (verbose);
